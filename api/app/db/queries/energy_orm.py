"""
Energy queries using SQLAlchemy ORM.
"""
...
from shared import FactEnergyHourly, FactEnergyDaily
...
def get_hourly(
    ...
    if datasource_id:
        query = query.filter(FactEnergyHourly.datasource_id == datasource_id)
...
def get_daily(
    ...
    if datasource_id:
        query = query.filter(FactEnergyDaily.datasource_id == datasource_id)
...
def get_stats(db: Session) -> dict:
    ...
    hourly_stats = db.query(
        ...
        func.count(func.distinct(FactEnergyHourly.datasource_id)).label('datasources')
    ).first()
    ...
    daily_stats = db.query(
        ...
        func.count(func.distinct(FactEnergyDaily.datasource_id)).label('datasources')
    ).first()
